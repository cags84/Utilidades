h1. Para la instalación se debe hacer lo siguiente:

* Se tiene como base un servidor con sistema operativo linux [Centos, Ubuntu, Debian, etc], con las herramientas básica e instalado GIT.

h2. Crear un directorio en el root del servidor
	
	@#mkdir /root/scripts@

h2. Ingresar al directorio

	@#cd /root/scripts@
 
h2. Instalamos GIT y solucionamos el problema con certificados https "ver":https://blog.breadncup.com/2011/06/09/skip-git-ssl-verification/

	@#apt-get install git - [Debian, Ubuntu]@
	@#export GIT_SSL_NO_VERIFY=1#
	
	"https://www.howtoforge.com/how-to-install-the-latest-git-version-on-centos":Centos y derivados
	
h2. Descargar los archivos

	@#git clone https://github.com/cags84/Utilidades/@

h2. Ingresamos a la carpeta del repositorio

	@#cd /root/scripts/Utilidades/Iptables/@

h2. Le damos permisos a los archivos

	@#chmod +x *.sh@

h2. Editamos los archivos y colocamos nuestras propias reglas

	@#vim /root/scripts/start.sh@
	@#vim /root/scripts/stop.sh@

h2. Cargamos al rc.local, para que arranque con la maquina:

	@#echo '/root/scripts/start.sh' >> /etc/rc.local@

h2. Como lo arrancamos desde la terminal?

	@#sh /root/scripts/start.sh@

h2. Como limpiamos las reglas y para el firewall?

	@#sh /root/scripts/stop.sh@
	
h2. Para ver las reglas

	@#iptables -L -nv@